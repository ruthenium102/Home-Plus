...
function HatAccessory({ id, cx, cy, r }: { id: string; cx: number; cy: number; r: number }) {
  if (id === 'beanie') {
    return (
      <g>
        <path
          d={`M ${cx - r * 0.55} ${cy + r * 0.1} Q ${cx} ${cy - r * 0.6} ${cx + r * 0.55} ${cy + r * 0.1} Z`}
          fill="#c44d2e"
          stroke="#8a3520"
          strokeWidth="1.5"
        />
  ...
  if (id === 'party') {
    return (
      <g>
        ...
        />
        <polygon
          points={`${cx - r * 0.32},${cy + r * 0.05} ${cx},${cy - r * 0.1} ${cx + r * 0.32},${cy + r * 0.05}`}
          fill="#f9a8d4"
          opacity="0.6"
        />
  ...
  if (id === 'glasses') {
    return (
      <g fill="none" stroke="#201c18" strokeWidth="2" strokeLinecap="round">
        ...
        <circle cx={cx + eyeSpacing} cy={eyeY} r={eyeR + 2} fill="rgba(180, 200, 220, 0.18)" />
        <line x1={cx - eyeSpacing + eyeR + 2} y1={eyeY} x2={cx + eyeSpacing - eyeR - 2} y2={eyeY} />
      </g>
    );
  }
  if (id === 'shades') {
    return (
      <g>
        ...
        <ellipse cx={cx + eyeSpacing} cy={eyeY} rx={eyeR + 3} ry={eyeR + 1} fill="#1a1a1a" />
        <line
          x1={cx - eyeSpacing + eyeR + 3}
          y1={eyeY}
          x2={cx + eyeSpacing - eyeR - 3}
          y2={eyeY}
          stroke="#1a1a1a"
          strokeWidth="2.5"
        />
        <line
          x1={cx - eyeSpacing - eyeR * 0.4}
          y1={eyeY - eyeR * 0.6}
          x2={cx - eyeSpacing + eyeR * 0.2}
          y2={eyeY - eyeR * 0.2}
          stroke="white"
          strokeWidth="2.5"
          strokeLinecap="round"
...
function NeckAccessory({ id, cx, cy, r }: { id: string; cx: number; cy: number; r: number }) {
  ...
  if (id === 'bow_tie') {
    return (
      <g>
        <polygon
          points={`${cx - r * 0.32},${neckY - r * 0.1} ${cx - r * 0.05},${neckY} ${cx - r * 0.32},${neckY + r * 0.1}`}
          fill="#dc2626"
          stroke="#7f1d1d"
          strokeWidth="1"
        />
